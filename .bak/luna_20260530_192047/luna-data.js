...
var LUNA = (function() {
  ...
  function completeMarker(orderId, markerData, operator, fabrics) {
    ...
    for (var i = 0; i < orders.length; i++) {
      if (orders[i].id === orderId) {
        ...
        var allDone = newFabrics.length > 0 && newFabrics.every(function(f){ return f.done; });
  ...
  function completeCutting(orderId, colorData, checkmarks, operator) {
    ...
    for (var i = 0; i < orders.length; i++) {
      if (orders[i].id === orderId) {
        ...
        var fabrics = {};
        var totalCut = 0;
        var firstFabric = null;
        for (var color in colorData) {
          var cd = colorData[color];
          if (!cd) continue;
          var fName = cd.fabric || '默认面料';
          if (!fabrics[fName]) {
            fabrics[fName] = { hands: cd.hands || 1, colors: {} };
            if (firstFabric === null) firstFabric = fName;
          }
          fabrics[fName].colors[color] = { layers: cd.layers || 1, total: cd.total || 0 };
          // 总件数只计第一种面料（用于月结），其余面料写入 fabrics 但不计入 total_cut
          if (fName === firstFabric) totalCut += cd.total || 0;
        }
        ...
        for (var fName in fabrics) {
          ...
          for (var mi = 0; mi < markerFabrics.length; mi++) {
            if (markerFabrics[mi].name === fName) { patternLen = markerFabrics[mi].length || 0; break; }
          }
          if (patternLen <= 0) continue;
          // 找到该面料的损耗值（默认 6cm = 0.06m）
          var lossCm = 6;
          ...
          // 合计该面料所有颜色的层数
          var totalLayers = 0;
          for (var c in fabrics[fName].colors) { totalLayers += fabrics[fName].colors[c].layers || 0; }
          var deduction = (patternLen + (lossCm / 100)) * totalLayers;
          // 扣减
          for (var fi = 0; fi < allFabrics.length; fi++) {
  ...
  function addToCart(code, name, color, qty, price, fabric, note) {
  ...
  function addFabricColor(fabricId, name, hex, imgPath) {
    var result = api('/api/fabrics/add-color', 'POST', {
      fabric_id: fabricId, name: name, hex: hex, img_path: imgPath || ''
    });
    if (result && result.ok) {
      // 直接更新缓存，不需要重新拉取全部面料
      if (_cache.fabrics) {
        for (var i = 0; i < _cache.fabrics.length; i++) {
          if (_cache.fabrics[i].id === fabricId) {
            ...
            _cache.fabrics[i].colors.push({name: name, hex: hex, img: imgPath || ''});
            break;
          }
        }
  ...
  return {
    ...
    login: login, canAccess: canAccess, checkAuth: checkAuth,
    ...
    _setCache: function(url, data) {
      if (url.indexOf('categories') >= 0) _cache.categories = data;
        ...
        // 防止 firstLoad 覆盖本地刚保存的 marker_complete 数据
        ...
        if (_cache.orders) {
          _cache.orders.forEach(function(o) {
            ...
            if (_old && _old.marker_complete && _old.marker_complete.completed === 1) {
              if (!o.marker_complete || o.marker_complete.completed === 0) {
                o.marker_complete = _old.marker_complete;
              }
            }
          });
        }
