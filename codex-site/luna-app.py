...
import json, os, sqlite3, uuid, re, base64, mimetypes, hashlib
...
def get_user():
    if 'user_id' in session:
        return {'username': session['user_id'], 'role': session.get('role'), 'name': session.get('name')}
    return None
...
def get_settings_list(key):
    ...
    db = get_db()
    if key == 'guests':
        rows = db.execute("SELECT id, username, name, phone, enabled FROM users WHERE role='guest'").fetchall()
...
def save_settings_list(key, data):
    ...
    if key == 'fabrics':
        ...
        for item in data:
            db.execute(
                "INSERT INTO fabrics (id, name, price_per_m, composition, stock) VALUES (?,?,?,?,?)",
        ...
        for item in data:
            db.execute(
                "INSERT OR REPLACE INTO users (id, username, password, name, role, phone, enabled) VALUES (?,?,?,?,?,?,?)",
                (item.get('id', 'g-'+uuid.uuid4().hex[:6]), item['username'],
                 item.get('password', ''), item.get('name', ''),
...
def save_single_order(order):
    ...
    db.execute("""INSERT OR REPLACE INTO orders
        (id, customer, date, total_qty, note,
         order_placed_completed, marker_completed, marker_length, marker_hands,
         marker_operator, marker_time,
         cutting_completed, cutting_total, cutting_operator, cutting_time,
         pickup_completed, pickup_factory, pickup_operator, pickup_time,
         shipping_completed, shipping_qty, shipping_operator, shipping_time)
        VALUES (?,?,?,?,?, ?,?,?,?, ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?)""",
        (o['id'], o.get('customer',''), o.get('date',''), o.get('total_qty',0), o.get('note',''),
    ...
    for item in o.get('items', []):
        db.execute(
            "INSERT INTO order_items (order_id, code, name, color, fabric, price, qty_data) VALUES (?,?,?,?,?,?,?)",
...
def save_single_style(style):
    ...
    db.execute("""INSERT OR REPLACE INTO styles
        ...
        VALUES (?,?,?,?,?,?, ?,?,?,?, ?,?,?)""",
        (s.get('code',''), s.get('name','未命名款式'), s.get('category',''),
    ...
    for i, img in enumerate(s.get('images', [])):
        # img can be base64 string or file path
        if img.startswith('data:') or img.startswith('/9j') or len(img) > 200:
            # It's base64 — we'll handle this in migration
            # For API saves, user should upload via /api/upload first
            pass
        else:
            db.execute(
...
def api_login():
    ...
    if username == 'admin' and password == 'admin':
        ...
        session['role'] = 'admin'
        session['name'] = '管理员'
        return jsonify({'username': 'admin', 'role': 'admin', 'name': '管理员'})
    # Check users table
...
def api_cart():
    ...
    if action == 'add':
        ...
        db.execute(
            ...
            (sid, data.get('code',''), data.get('name',''),
             data.get('color',''), data.get('fabric',''),
             data.get('price',0), data.get('note',''),
...
def api_export_csv():
    ...
    header = ['订单号','编号','颜色','下单备注'] + size_keys + ['订单总数','裁剪总数','发货总数']
    ...
    for o in result:
        ...
        if o.get('items'):
            for item in o['items']:
                row = [o['id'], item.get('code',''), item.get('color',''), order_note]
                item_total = 0
                for s in size_keys:
...
if __name__ == '__main__':
    with app.app_context():
        init_db()
    print(f'LUNA Flask Server on http://0.0.0.0:8765')
